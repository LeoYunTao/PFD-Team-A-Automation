...
class TestApplyAccountData:
    @staticmethod
    def generate_form_data(rows=5):
        fake = Faker()
        return pd.DataFrame({
            'Nickname': [fake.unique.nick_name() for _ in range(rows)],      #GENERATES RANDOM NICK NAME AND TYPE OF ACC
            'accType': [random.randint(0, 1) for _ in range(rows)],
        })
...
class TestApplyAccount:
    def test_main(self, driver, form_input_data):
        for test_range in range(len(list(form_input_data())[0])):
            ...
            form_fields = driver.find_elements(By.CSS_SELECTOR, 'input, select') #FINDS input and select dropdown type

            for form_field in form_fields:
                if form_field.get_attribute('name') not in form_input_data:
                    continue
                value_to_enter = form_input_data[form_field.get_attribute('name')]
